...
A one-file, dependency-free client for [Jude's Tasks app](https://tasks.judemakes.dev) — the personal, Things-style to-do list that syncs between his Mac and phone. It lets a bot or script **list, add, complete and trash** to-dos through the sync server's JSON API.

It needs a **bot key**, which Jude hands out separately. The key can do those four things and nothing else.
...
- **Only complete or remove what Jude asked you to.** If a title matches more than one open to-do the script refuses and shows the candidates; pick by id. `remove` moves to Trash, which Jude can restore from the app.
